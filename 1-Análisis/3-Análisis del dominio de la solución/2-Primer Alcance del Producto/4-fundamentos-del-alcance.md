...
| `FND-CT-06` | Diferencia entre contacto y usuario | Los contactos institucionales son registros informativos y no roles operativos del sistema. | Un contacto de tipo legal, técnico o de búsqueda inmediata no debe recibir acceso automático a la plataforma por existir como dato institucional. |
...
| `FND-VR-05` | Estado operativo de la institución | La condición vigente de la institución dentro del SaaS expresada mediante `planStatus`. | Backoffice, operación institucional, configuración visible y decisiones administrativas. |
...
| `INSTITUTION_CONTACT_TYPE` | `LEGAL`, `TECHNICAL`, `IMMEDIATE_SEARCH` | Tipología de contactos institucionales. | Debe distinguir contactos informativos sin convertirlos en roles operativos. |
...
| `COMMERCIAL_PLAN_STATUS` | `ACTIVE`, `WARNING`, `PAUSED`, `STOPPED` | Estado de vigencia operativa de la institución dentro del SaaS. | Debe expresar si la institución opera normalmente, opera con advertencia visible, queda pausada o deja de estar habilitada para continuar el servicio. |
| `LOG_CATEGORIES` | `INSTITUTION_CREATION`, `INSTITUTION_PLAN_CREATION`, `INSTITUTION_PLAN_UPDATE`, `INSTITUTION_CONTACT_CREATION`, `INSTITUTION_CONTACT_UPDATE`, `INSTITUTION_SHARED_SECRET_UPDATE`, `INSTITUTION_PERMISSION_CREATION`, `INSTITUTION_PERMISSION_UPDATE`, `INSTITUTION_SEARCH_REQUEST_PHASE_CHANGE`, `INSTITUTION_SEARCH_REQUEST_PHASE_STATUS_CHANGE`, `INSTITUTION_SEARCH_REQUEST_MATCH_FOUND`, `PUI_SEARCH_REQUEST_CREATION`, `PUI_SEARCH_REQUEST_REVOCATION`, `PUI_SEARCH_REQUEST_HISTORICAL_FINISH`, `PUI_SEARCH_REQUEST_MATCH`, `PUI_SEARCH_REQUEST_API_ERROR`, `PUI_FINDING_SYNC_SUCCESS`, `PUI_FINDING_SYNC_ERROR`, `PUI_FINDING_SYNC_RESCHEDULE`, `USER_ACCOUNT_CREATION`, `USER_ACCOUNT_LOGIN`, `USER_ACCOUNT_LOGOUT`, `USER_ACCOUNT_PASSWORD_RECOVERY_REQUEST`, `USER_ACCOUNT_PASSWORD_UPDATE`, `USER_ACCOUNT_EMAIL_VERIFICATION`, `USER_ACCOUNT_MFA_ENROLL`, `USER_ACCOUNT_MFA_UNENROLL`, `USER_ACCOUNT_SETTINGS_UPDATE` | Clasificación funcional de eventos de log. | Debe permitir filtrar, agrupar y leer la trazabilidad sin ambigüedad semántica. |
...
| `FND-RG-08A` | La vigencia operativa de una institución dentro del SaaS debe modelarse mediante `planStatus`. | El estado institucional no debe inferirse implícitamente solo a partir de fechas o de texto libre. |
...
| `FND-RG-12A` | El proveedor SaaS usa el mismo modelo de permisos bajo `SYSTEM_RFC`. | `SYSTEM_ADMINISTRATOR` no debe modelarse con un mecanismo paralelo ajeno a `Permissions`, sino como contexto sistémico reservado dentro del mismo esquema de acceso. |
...
| `FND-CR-09A` | Evidencia operativa de sincronización PUI | Los intentos de sincronización de `Findings` deben conservar evidencia explícita de fecha, código HTTP y respuesta devuelta por la PUI, sin sustituir el estado vigente de la entidad. | `Findings`, backend, soporte operativo y auditoría de interoperabilidad. |
