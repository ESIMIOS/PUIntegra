  ...
  [DOMAIN.SITE]: [
    {
      ...
      label: 'Home',
      icon: 'home',
      description: 'Página pública principal del producto con información de valor, acceso y contacto.',
    ...
    },
    {
      ...
      label: 'Demo',
      icon: 'palette',
      description: 'Referencia viva del sistema visual, tokens y patrones de componentes.',
      to: () => routePaths.siteDemo,
    },
  ],
  [DOMAIN.AUTH]: [
    {
...
export function buildNavigationBackLinks(domain: NavigationDomain, context: NavigationContext): NavigationLink[] {
  ...
  if (domain == DOMAIN.ACCOUNT) {
    ...
    if (context.isInstitutionRole) {
      domainLinks.push({
        id: 'back-to-app',
        label: 'Volver a app',
        icon: 'dashboard',
        to: routePaths.appDashboard(context.activeRfc),
        disabled: false,
      });
    }
  }
